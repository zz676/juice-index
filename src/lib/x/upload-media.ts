/**
 * Upload an image to X via the v1.1 Media Upload endpoint (OAuth 1.0a).
 *
 * The v2 media upload endpoint is not generally available at Basic API access.
 * The v1.1 endpoint supports OAuth 1.0a only (rejects OAuth 2.0 Bearer tokens).
 *
 * We sign with the app-level OAuth 1.0a credentials, but pass the posting
 * user's X user ID in `additional_owners` so that X grants that user ownership
 * of the returned media_id.  The user can then attach the media_id to their
 * own tweet posted via OAuth 2.0.
 */
...
export async function uploadMedia(
  _accessToken: string,
  base64Data: string,
  options?: { additionalOwner?: string }
  ...
  // Grant the posting user ownership so their OAuth 2.0 token can attach this
  // media_id to their own tweet (media_ids are user-scoped on X).
  if (options?.additionalOwner) {
    form.append("additional_owners", options.additionalOwner);
  }
