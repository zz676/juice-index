...
export async function GET(request: NextRequest) {
  ...
  const endpoint = "/api/v1/industry/dealer-inventory-factor";
  ...
  try {
    const [data, total] = await Promise.all([
      (prisma as any).chinaDealerInventoryFactor.findMany({
        where,
        orderBy: [{ year: sortOrder }, { month: sortOrder }],
        skip,
        take: limit,
      }),
      (prisma as any).chinaDealerInventoryFactor.count({ where }),
    ]);
